...
namespace SmartEcoAPI.Akimato.Controllers
{
    ...
    public class LEDDisplayController : ControllerBase
    {
        ...
        [HttpGet(nameof(GetLedWebDatas))]
        public async Task<JsonResult> GetLedWebDatas(string namePosts)
        {
            try
            {
                var measuredDatas = await _context.MeasuredData
                    ...
                    .Include(m => m.MeasuredParameter)
                    .Include(m => m.MeasuredParameter.MeasuredParameterUnit)
                    .Where(m => m.DateTime != null && m.Averaged == true && m.DateTime > DateTime.Now.AddHours(-1) && m.MonitoringPost.Name.Contains(namePosts))
                    .OrderByDescending(m => m.DateTime)
                    .ToListAsync();
                ...
                var mpmp = await _context.MonitoringPostMeasuredParameters
                    ...
                    .Include(m => m.MeasuredParameter)
                    .Include(m => m.MeasuredParameter.MeasuredParameterUnit)
                    .Where(m => m.MonitoringPost.Name.Contains(namePosts))
                    .ToListAsync();
