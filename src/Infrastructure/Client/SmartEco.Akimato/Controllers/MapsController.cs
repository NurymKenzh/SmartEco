...
using SmartEco.Akimato.Models.UPRZA;
...
namespace SmartEco.Akimato.Controllers
{
    public class MapsController : Controller
    {
        ...
        public class AlmatyMapModel
        {
            public TypeMap Type { get; set; }
        }

        public async Task<IActionResult> AlmatyMap(TypeMap type)
        {
            ...
            List<MonitoringPost> clarityAirMonitoringPosts = monitoringPosts
                .Where(m => m.Project != null && m.Project.Name == "Almaty"
                && m.DataProviderId == 4 //Clarity
                && m.TurnOnOff == true)
                .ToList();

            JObject clarityAirMonitoringPostsObject = GetObjectEcoserviceAirMonitoringPosts(decimaldelimiter, clarityAirMonitoringPosts);
            ViewBag.ClarityAirMonitoringPostsLayerJson = clarityAirMonitoringPostsObject.ToString();

            ViewBag.ClarityAirMonitoringPosts = clarityAirMonitoringPosts.ToArray();

            (ViewBag.LEDScreensId, ViewBag.LEDScreensAQI) = await GetAQIPosts("Almaty");

            return View(new AlmatyMapModel 
            { 
                Type = type
            });
        }
        ...
        public async Task<IActionResult> GetMeasuredParameters(int MonitoringPostId)
        {
            ...
            if (result[0].MonitoringPost.DataProviderId == 3)
            {
