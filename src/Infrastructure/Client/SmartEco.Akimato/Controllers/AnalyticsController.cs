...
namespace SmartEco.Akimato.Controllers
{
    public class AnalyticsController : Controller
    {
        ...
        public async Task<IActionResult> GetMonitoringPosts(string projectName, bool akimato = false)
        {
            var result = await GetAllMonitoringPosts(projectName, akimato);
        ...
        public async Task<List<MonitoringPost>> GetAllMonitoringPosts(string projectName, bool akimato = false)
        {
            string urlMonitoringPosts = "api/MonitoringPosts";
            List<MonitoringPost> monitoringPosts = new List<MonitoringPost>();
            HttpResponseMessage responseMonitoringPosts = await _HttpApiClient.GetAsync(urlMonitoringPosts);
            ...
            if (akimato)
            {
                return monitoringPosts
                    .Where(m => m.Project != null && m.Project.Name == projectName
                        && m.TurnOnOff == true
                        && m.PollutionEnvironmentId == 2)
                    .OrderBy(m => m.DataProviderId)
                    .ThenBy(m => m.AdditionalInformation)
                    .ToList();
            }
            else
            {
